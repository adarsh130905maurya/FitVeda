...
const LoginPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Phase 1 / Phase 2 dev/mock credentials
    setTimeout(() => {
      if (email && password) {
        login({
          token: 'dev-token-' + Math.random(),
          role: role,
          userId: role === 'TRAINER' ? '1' : '2',
          name: role === 'TRAINER' ? 'Coach Pari' : 'Client John',
          email: email
        });
        window.location.href = role === 'TRAINER' ? '/trainer' : '/client';
      } else {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md z-10 animate-slide-down">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 font-medium">Empowering your health and fitness journey</p>
        </div>
        ...
        <Card className="shadow-xl shadow-slate-100 border border-slate-100/50">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <Input
              ...
              id="email"
              type="email"
              placeholder="e.g. pari@fitveda.com"
              value={email}
